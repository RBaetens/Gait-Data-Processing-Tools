...
dataRec1 = strcat(dataFolder, '\FP4A_cal_data_02.c3d');
dataRec2 = strcat(dataFolder, '\FP4A_cal_data_03.c3d'); % validation
...
% Force plates
PILS_meas_mat = get_fp_fm_meas(analog_signals, 4);
cop_plate = get_fp_cop_meas(analog_signals, corners_force_plates, 4);
Ftot_plate = sqrt(PILS_meas_mat(:, 1).^2 + PILS_meas_mat(:, 2).^2 + PILS_meas_mat(:, 3).^2);

% Rod
PILS_ref_mat = get_pole_fm_meas(analog_signals, markers, corners_force_plates, 4);
...
S_val_corr = C * S_val;
cop_fp = get_cop_from_fm(S_val, corners_force_plates, 4);
cop_rod = get_cop_from_fm(R_val, corners_force_plates, 4);
cop_fp_corr = get_cop_from_fm(S_val_corr, corners_force_plates, 4);
...
writematrix(C, strcat(saveFolder, '\C_matrix_FP4A.txt'))
